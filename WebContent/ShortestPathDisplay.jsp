...
<html>
	<head> <%-- Head of the JSP --%>
		...
		<title>Shortest Path</title> <%-- Title of the Web Page --%>
	...
	<body>
		...
		<main>
			...
			<table id="table" >
  				<tr>
    				<th><h1><i>Shortest Path :</i></h1></th>
  				</tr>
  				<tr>
  					<th>
  						<% 
  						int source = (int)request.getServletContext().getAttribute("source");
  						int destination = (int)request.getServletContext().getAttribute("destination");
  						List<Object> router_List = (List<Object>)request.getServletContext().getAttribute("routerList");
  						int temp_matrix_destination = (int)request.getServletContext().getAttribute("matrix_destination");
  						int temp_matrix_source= (int)request.getServletContext().getAttribute("matrix_source");
  						
  						out.print("<h3><b><i>Shortest path from Router " + (source+1) + " to Router " + (destination+1) + " is as below:</i></b></h3>");
  						for (int i = 0; i < router_List.size(); i++) {
  							out.print("Router "+router_List.get(i)+" to " );
  						}
  						out.print("Router "+(destination+1)+" .");
  						out.println("<br>");
  						if(temp_matrix_destination - temp_matrix_source!=-1){
  						out.println("<h3><b><i>The total cost is "+ (temp_matrix_destination - temp_matrix_source) + ".<h3><b><i>");
  						}
  						else{
  							out.println("<h3><b><i>There is no direct path<h3><b><i>");
  	  					}
  						%>
  						<%-- Creation of a form, the actions executed after an event--%>
